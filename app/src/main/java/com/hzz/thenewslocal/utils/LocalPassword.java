...
public class LocalPassword {
    ...
    public static void savePassword(Context context, String number, String password, boolean isRemember) {
        //对数据进行加密
        //得到key
        SecretKey key = LocalPassword.readKey(LocalPassword.getPath("a.a"));
        if (key == null) {
            key = get3DESKey();
            //保存key
            LocalPassword.saveKey(key, LocalPassword.getPath("a.a"));
        }
    ...
    private static String d(String str) {
        if (!TextUtils.isEmpty(str)) {
            //对数据进行解密
            SecretKey key = readKey(LocalPassword.getPath("a.a"));
            if (key != null) {
